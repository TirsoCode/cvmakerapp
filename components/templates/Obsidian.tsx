...
export default function ObsidianTemplate({ data }: Props) {
  ...
  return (
    <div style={{ fontFamily: getBodyFontFamily(data.settings), color: "#FAFAFA", background: "#18181B", padding: p3644 }}>
      <header style={{ marginBottom: p28, paddingBottom: p20, borderBottom: `1px solid #3F3F46` }}>
        <h1 style={{ fontFamily: getFontFamily(data.settings), fontSize: 26*s, fontWeight: 800, letterSpacing: "-0.03em", margin: `0 0 ${4*s}px`, color: "#FAFAFA" }}>{personal.name || "Nombre"}</h1>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: p416, fontSize: 11*s, color: "#A1A1AA" }}>
          {personal.email && <span>{personal.email}</span>}{personal.phone && <span>{personal.phone}</span>}{personal.location && <span>{personal.location}</span>}{personal.linkedin && <span>{personal.linkedin}</span>}{personal.github && <span>{personal.github}</span>}{(personal.website || personal.portfolio) && <span>{personal.website || personal.portfolio}</span>}
        </div>
      </header>
      ...
      {data.settings.sections.experience && experience.length > 0 && <section style={{ marginBottom: p24 }}>
        <h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p14}` }}>Experiencia</h2>
      ...
      {data.settings.sections.education && education.length > 0 && <section style={{ marginBottom: p24 }}>
        <h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p14}` }}>Educación</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: `0px ${32*s}px` }}>
        {data.settings.sections.skills && skills.length > 0 && <section><h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Habilidades</h2>{skills.map((cat) => <div key={cat.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 700, color: "#71717A" }}>{cat.category}: </span><span style={{ fontSize: 11*s, color: "#D4D4D8" }}>{cat.items.filter(Boolean).join(", ")}</span></div>)}</section>}
        <div>{data.settings.sections.languages && languages.length > 0 && <section style={{ marginBottom: `${20*s}px` }}><h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Idiomas</h2>{languages.map((l) => <div key={l.id} style={{ fontSize: 11*s, marginBottom: p4 }}><span style={{ fontWeight: 700, color: "#FAFAFA" }}>{l.language}</span> <span style={{ color: "#71717A" }}>{l.level}</span></div>)}</section>}
          {data.settings.sections.projects && projects.length > 0 && <section><h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Proyectos</h2>{projects.map((p) => <div key={p.id} style={{ marginBottom: p8 }}><h3 style={{ fontSize: 11*s, fontWeight: 700, margin: `0 0 ${2*s}px`, color: "#FAFAFA" }}>{p.name}</h3><p style={{ fontSize: 10*s, color: "#A1A1AA", margin: 0 }}>{p.description}</p></div>)}</section>}</div>
      </div>
      {data.settings.sections.certifications && certifications.length > 0 && <section style={{ marginBottom: p24 }}>
        ...
        {certifications.map((c) => <div key={c.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#FAFAFA" }}>{c.name}</span> <span style={{ fontSize: 10*s, color: "#71717A" }}>— {c.issuer} {c.date ? ` (${c.date})` : ""}</span></div>)}
      </section>}
      {data.settings.sections.awards && awards.length > 0 && <section style={{ marginBottom: p24 }}>
        ...
        {awards.map((a) => <div key={a.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#FAFAFA" }}>{a.name}</span> <span style={{ fontSize: 10*s, color: "#71717A" }}>— {a.issuer} {a.date ? ` (${a.date})` : ""}</span></div>)}
      </section>}
      {data.settings.sections.licenses && licenses.length > 0 && <section style={{ marginBottom: p24 }}>
        ...
        {licenses.map((l) => <div key={l.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#FAFAFA" }}>{l.name}</span> <span style={{ fontSize: 10*s, color: "#71717A" }}>— {l.issuer} {l.licenseNumber ? ` (${l.licenseNumber})` : ""}</span></div>)}
      </section>}
      {data.settings.sections.references && references.length > 0 && <section style={{ marginBottom: p24 }}>
        ...
        {references.map((r) => <div key={r.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#FAFAFA" }}>{r.name}</span> <span style={{ fontSize: 10*s, color: "#71717A" }}>— {r.company} {r.relationship ? ` (${r.relationship})` : ""}</span><br /><span style={{ fontSize: 10*s, color: "#71717A" }}>{[r.email, r.phone].filter(Boolean).join(" | ")}</span></div>)}
      </section>}
      {data.settings.sections.affiliations && affiliations.length > 0 && <section style={{ marginBottom: p24 }}>
        ...
        {affiliations.map((a) => <div key={a.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#FAFAFA" }}>{a.organization}</span> <span style={{ fontSize: 10*s, color: "#71717A" }}>— {a.role} {a.startDate || a.endDate ? ` (${a.startDate}${a.startDate && a.endDate ? " — " : ""}${a.endDate})` : ""}</span></div>)}
      </section>}
