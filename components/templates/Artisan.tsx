  ...
  return (
    <div style={{ fontFamily: getBodyFontFamily(data.settings), color: "#2C1810", background: "#FFFDF8", padding: p4048 }}>
      ...
      {data.settings.sections.certifications && certifications.length > 0 && <section style={{ marginBottom: p28 }}>
        ...
        {certifications.map((c) => <div key={c.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#5C4A3A" }}>{c.name}</span> <span style={{ fontSize: 10*s, color: "#8B7355" }}>— {c.issuer} {c.date ? ` (${c.date})` : ""}</span></div>)}
      </section>}
      {data.settings.sections.awards && awards.length > 0 && <section style={{ marginBottom: p28 }}>
        ...
        {awards.map((a) => <div key={a.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#5C4A3A" }}>{a.name}</span> <span style={{ fontSize: 10*s, color: "#8B7355" }}>— {a.issuer} {a.date ? ` (${a.date})` : ""}</span></div>)}
      </section>}
      {data.settings.sections.licenses && licenses.length > 0 && <section style={{ marginBottom: p28 }}>
        ...
        {licenses.map((l) => <div key={l.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#5C4A3A" }}>{l.name}</span> <span style={{ fontSize: 10*s, color: "#8B7355" }}>— {l.issuer} {l.licenseNumber ? ` (${l.licenseNumber})` : ""}</span></div>)}
      </section>}
      {data.settings.sections.references && references.length > 0 && <section style={{ marginBottom: p28 }}>
        ...
        {references.map((r) => <div key={r.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#5C4A3A" }}>{r.name}</span> <span style={{ fontSize: 10*s, color: "#8B7355" }}>— {r.company} {r.relationship ? ` (${r.relationship})` : ""}</span><br /><span style={{ fontSize: 10*s, color: "#8B7355" }}>{[r.email, r.phone].filter(Boolean).join(" | ")}</span></div>)}
      </section>}
      {data.settings.sections.affiliations && affiliations.length > 0 && <section style={{ marginBottom: p28 }}>
        ...
        {affiliations.map((a) => <div key={a.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 11*s, fontWeight: 600, color: "#5C4A3A" }}>{a.organization}</span> <span style={{ fontSize: 10*s, color: "#8B7355" }}>— {a.role} {a.startDate || a.endDate ? ` (${a.startDate}${a.startDate && a.endDate ? " — " : ""}${a.endDate})` : ""}</span></div>)}
      </section>}
