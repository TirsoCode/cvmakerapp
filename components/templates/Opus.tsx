...
export default function OpusTemplate({ data }: Props) {
  ...
  return (
    <div style={{ fontFamily: getBodyFontFamily(data.settings), color: "#1A1918", background: "#FEFCE8", padding: p4048 }}>
      <header style={{ marginBottom: p32, borderBottom: `1px solid #CA8A04` }}>
        <h1 style={{ fontFamily: getFontFamily(data.settings), fontSize: 28*s, fontWeight: 700, letterSpacing: "-0.02em", margin: p004, color: "#713F12" }}>{personal.name || "Nombre"}</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: `0px ${p32}` }}>
        {data.settings.sections.skills && skills.length > 0 && <section><h2 style={{ fontSize: 10*s, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: p0012 }}>Habilidades</h2>{skills.map((cat) => <div key={cat.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 10*s, fontWeight: 700, color: "#A16207" }}>{cat.category}: </span><span style={{ fontSize: 11*s, color: "#854D0E" }}>{cat.items.filter(Boolean).join(", ")}</span></div>)}</section>}
        <div>
