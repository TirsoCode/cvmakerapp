  ...
  return (
    <div style={{ fontFamily: getBodyFontFamily(data.settings), color: "#FAFAFA", background: "#111", padding: p3644 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: p0_32 }}>
        {data.settings.sections.skills && skills.length > 0 && <section><h2 style={{ fontSize: 10*s, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: p0012 }}>Habilidades</h2>{skills.map((cat) => <div key={cat.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 10*s, fontWeight: 700, color: "#888" }}>{cat.category}: </span><span style={{ fontSize: 11*s, color: "#AAA" }}>{cat.items.filter(Boolean).join(", ")}</span></div>)}</section>}
        <div>
