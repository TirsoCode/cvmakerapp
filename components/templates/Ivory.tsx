...
export default function IvoryTemplate({ data }: Props) {
  ...
  return (
    <div style={{ fontFamily: getBodyFontFamily(data.settings), color: "#44403C", background: "#FDFDFA", padding: p4452 }}>
      <header style={{ marginBottom: p32, textAlign: "center" }}>
        <h1 style={{ fontFamily: getFontFamily(data.settings), fontSize: 30*s, fontWeight: 700, letterSpacing: "-0.02em", margin: `0 0 ${6*s}px`, color: "#1C1917" }}>{personal.name || "Nombre"}</h1>
      ...
      {data.settings.sections.experience && experience.length > 0 && <section style={{ marginBottom: p28 }}>
        <h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p16}`, textAlign: "center" }}>Experiencia Profesional</h2>
      ...
      {data.settings.sections.education && education.length > 0 && <section style={{ marginBottom: p28 }}>
        <h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p16}`, textAlign: "center" }}>Formación Académica</h2>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: `0px ${p32}` }}>
        {data.settings.sections.skills && skills.length > 0 && <section><h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Habilidades</h2>{skills.map((cat) => <div key={cat.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 10*s, fontWeight: 700, color: "#78716C" }}>{cat.category}: </span><span style={{ fontSize: 11*s, color: "#57534E" }}>{cat.items.filter(Boolean).join(", ")}</span></div>)}</section>}
        <div>{data.settings.sections.languages && languages.length > 0 && <section style={{ marginBottom: `${20*s}px` }}><h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Idiomas</h2>{languages.map((l) => <div key={l.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 11*s, marginBottom: p4 }}><span style={{ fontWeight: 600, color: "#57534E" }}>{l.language}</span><span style={{ color: "#78716C" }}>{l.level}</span></div>)}</section>}
          {data.settings.sections.projects && projects.length > 0 && <section><h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Proyectos</h2>{projects.map((p) => <div key={p.id} style={{ marginBottom: p8 }}><h3 style={{ fontSize: 11*s, fontWeight: 600, margin: `0 0 ${2*s}px` }}>{p.name}</h3><p style={{ fontSize: 10*s, color: "#57534E", margin: 0, lineHeight: 1.5 }}>{p.description}</p></div>)}</section>}
          {data.settings.sections.certifications && certifications.length > 0 && <section><h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Certificaciones</h2>{certifications.map((c) => <div key={c.id} style={{ marginBottom: p8 }}><h3 style={{ fontSize: 11*s, fontWeight: 600, margin: `0 0 ${2*s}px` }}>{c.name}</h3><p style={{ fontSize: 10*s, color: "#57534E", margin: 0 }}>{[c.issuer, c.date].filter(Boolean).join(" — ")}</p></div>)}</section>}
          {data.settings.sections.awards && awards.length > 0 && <section><h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Premios</h2>{awards.map((a) => <div key={a.id} style={{ marginBottom: p8 }}><h3 style={{ fontSize: 11*s, fontWeight: 600, margin: `0 0 ${2*s}px` }}>{a.name}</h3><p style={{ fontSize: 10*s, color: "#57534E", margin: 0 }}>{[a.issuer, a.date].filter(Boolean).join(" — ")}</p></div>)}</section>}
          {data.settings.sections.licenses && licenses.length > 0 && <section><h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Licencias</h2>{licenses.map((l) => <div key={l.id} style={{ marginBottom: p8 }}><h3 style={{ fontSize: 11*s, fontWeight: 600, margin: `0 0 ${2*s}px` }}>{l.name}</h3><p style={{ fontSize: 10*s, color: "#57534E", margin: 0 }}>{[l.issuer, l.licenseNumber, l.date].filter(Boolean).join(" — ")}</p></div>)}</section>}
          {data.settings.sections.references && references.length > 0 && <section><h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Referencias</h2>{references.map((r) => <div key={r.id} style={{ marginBottom: p8 }}><h3 style={{ fontSize: 11*s, fontWeight: 600, margin: `0 0 ${2*s}px` }}>{r.name}</h3><p style={{ fontSize: 10*s, color: "#57534E", margin: 0 }}>{[r.company, r.relationship].filter(Boolean).join(" — ")}</p><p style={{ fontSize: 10*s, color: "#57534E", margin: 0 }}>{[r.phone, r.email].filter(Boolean).join(" — ")}</p></div>)}</section>}
          {data.settings.sections.affiliations && affiliations.length > 0 && <section><h2 style={{ fontSize: 11*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Afiliaciones</h2>{affiliations.map((a) => <div key={a.id} style={{ marginBottom: p8 }}><h3 style={{ fontSize: 11*s, fontWeight: 600, margin: `0 0 ${2*s}px` }}>{a.organization}</h3><p style={{ fontSize: 10*s, color: "#57534E", margin: 0 }}>{[a.role, a.startDate, a.endDate].filter(Boolean).join(" — ")}</p></div>)}</section>}
        </div>
      </div>
