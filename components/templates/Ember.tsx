  ...
  return (
    <div style={{ fontFamily: getBodyFontFamily(data.settings), color: "#1A1918", background: "#fff", padding: p3644 }}>
      <header style={{ marginBottom: p28, borderLeft: `4px solid ${accentColor}`, paddingLeft: p16 }}>
        <h1 style={{ fontFamily: getFontFamily(data.settings), fontSize: 26*s, fontWeight: 800, letterSpacing: "-0.03em", margin: `0 0 ${4*s}px`, color: "#1A1918" }}>{personal.name || "Nombre"}</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: `0px ${32*s}px` }}>
        {data.settings.sections.skills && skills.length > 0 && <section><h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Habilidades</h2>{skills.map((cat) => <div key={cat.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 10*s, fontWeight: 700, color: "#6B6860" }}>{cat.category}: </span><span style={{ fontSize: 11*s, color: "#4A4843" }}>{cat.items.filter(Boolean).join(", ")}</span></div>)}</section>}
