...
export default function SlateTemplate({ data }: Props) {
  ...
  return (
    <div style={{ fontFamily: getBodyFontFamily(data.settings), color: "#1A1918", background: "#F8FAFC", padding: p3644 }}>
      <header style={{ marginBottom: p28, borderBottom: `2px solid ${accentColor}`, paddingBottom: p20 }}>
        <h1 style={{ fontFamily: getFontFamily(data.settings), fontSize: 26*s, fontWeight: 800, letterSpacing: "-0.03em", margin: p004, color: "#1E293B" }}>{personal.name || "Nombre"}</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: p0_32 }}>
        {data.settings.sections.skills && skills.length > 0 && <section><h2 style={{ fontSize: 10*s, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.14em", color: accentColor, margin: `0 0 ${p12}` }}>Habilidades</h2>{skills.map((cat) => <div key={cat.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 10*s, fontWeight: 700, color: "#475569" }}>{cat.category}: </span><span style={{ fontSize: 11*s, color: "#64748B" }}>{cat.items.filter(Boolean).join(", ")}</span></div>)}</section>}
        <div>
