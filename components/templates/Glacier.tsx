...
export default function GlacierTemplate({ data }: Props) {
  ...
  return (
    <div style={{ fontFamily: getBodyFontFamily(data.settings), color: "#1A1918", background: "#F0F9FF", padding: p4048 }}>
      <header style={{ marginBottom: p32, borderBottom: `1px solid #BAE6FD` }}>
        <h1 style={{ fontFamily: getFontFamily(data.settings), fontSize: 28*s, fontWeight: 800, letterSpacing: "-0.03em", margin: `0 0 ${4*s}px`, color: "#0C4A6E" }}>{personal.name || "Nombre"}</h1>
      ...
      {data.settings.sections.experience && experience.length > 0 && <section style={{ marginBottom: p26 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10*s, marginBottom: p14 }}><span style={{ display: "block", width: 24*s, height: 2*s, background: accentColor, borderRadius: 1 }} /><h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: "#0369A1", margin: 0 }}>Experiencia</h2></div>
        {experience.map((item) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: `0px ${p32}` }}>
        {data.settings.sections.skills && skills.length > 0 && <section><div style={{ display: "flex", alignItems: "center", gap: 10*s, marginBottom: p12 }}><span style={{ display: "block", width: 24*s, height: 2*s, background: accentColor, borderRadius: 1 }} /><h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: "#0369A1", margin: 0 }}>Habilidades</h2></div>{skills.map((cat) => <div key={cat.id} style={{ marginBottom: p8 }}><span style={{ fontSize: 10*s, fontWeight: 700, color: "#0369A1" }}>{cat.category} — </span><span style={{ fontSize: 11*s, color: "#0C4A6E" }}>{cat.items.filter(Boolean).join(", ")}</span></div>)}</section>}
        <div>
          {data.settings.sections.languages && languages.length > 0 && <section style={{ marginBottom: `${20*s}px` }}><div style={{ display: "flex", alignItems: "center", gap: 10*s, marginBottom: p12 }}><span style={{ display: "block", width: 24*s, height: 2*s, background: accentColor, borderRadius: 1 }} /><h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: "#0369A1", margin: 0 }}>Idiomas</h2></div>{languages.map((l) => <div key={l.id} style={{ fontSize: 11*s, marginBottom: p4 }}><span style={{ fontWeight: 700 }}>{l.language}</span> <span style={{ color: "#0369A1" }}>{l.level}</span></div>)}</section>}
          {data.settings.sections.projects && projects.length > 0 && <section><div style={{ display: "flex", alignItems: "center", gap: 10*s, marginBottom: p12 }}><span style={{ display: "block", width: 24*s, height: 2*s, background: accentColor, borderRadius: 1 }} /><h2 style={{ fontSize: 10*s, fontWeight: 700, fontFamily: getFontFamily(data.settings), textTransform: "uppercase", letterSpacing: "0.14em", color: "#0369A1", margin: 0 }}>Proyectos</h2></div>{projects.map((p) => <div key={p.id} style={{ marginBottom: p8 }}><h3 style={{ fontSize: 11*s, fontWeight: 700, margin: `0 0 ${2*s}px` }}>{p.name}</h3><p style={{ fontSize: 10*s, color: "#0C4A6E", margin: 0 }}>{p.description}</p></div>)}</section>}
        </div>
      </div>
